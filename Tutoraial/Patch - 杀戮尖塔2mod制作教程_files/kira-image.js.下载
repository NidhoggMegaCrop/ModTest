window.addEventListener('load', () => {
	...
	const imageRoot = document.querySelector('.kira-image');
	// Move overlay root to body to avoid clipping by parent containers.
	if (imageRoot && imageRoot.parentElement !== document.body) {
	...
	const setImageProp = () => {
		...
		modal.querySelector('.kira-image-header > .kira-image-counter').innerText = `${index + 1} / ${imgProps.length
			}`;
	...
	const prevButton = document.querySelector(
		'.kira-image div.kira-image-container > div.kira-image-prev-button-panel > div'
	);
	const nextButton = document.querySelector(
		'.kira-image div.kira-image-container > div.kira-image-next-button-panel > div'
	);
